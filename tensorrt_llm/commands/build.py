...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to read timing cache, will be ignored if the file does not exist",
    )
    parser.add_argument(
        "--output_timing_cache",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="This option is introduced with TensorRT 9.1.0.1+ and will reduce the engine building time. "
    ...
    parser.add_argument(
        "--logits_dtype", type=str, default=None, choices=["float16", "float32"]
    )
    parser.add_argument(
        "--weight_only_precision", type=str, default=None, choices=["int8", "int4"]
    )
    ...
    parser.add_argument(
        "--auto_parallel", type=int, default=1, help="MPI world size for auto parallel."
    )
    parser.add_argument(
        "--gpus_per_node",
        type=int,
        default=8,
        help="Number of GPUs each node has in a multi-node setup. This is a cluster spec and can be greater/smaller than world size",
    )
    parser.add_argument(
        "--cluster_key",
        type=str,
        ...
        help="Unique name for target GPU type. Inferred from current GPU type if not specified.",
    )
    parser.add_argument(
        "--strip_plan",
        default=False,
        action="store_true",
        help="Whether to strip weights from the final TRT engine under the assumption that the refit weights will be identical to those provided at build time.",
    )
    parser.add_argument(
        "--max_encoder_input_len",
        type=int,
        default=1024,
        help="Specify max encoder input length when using enc-dec models. Set max_input_len to 1 to start generation from decoder_start_token_id of length 1.",
    )
    parser.add_argument(
        "--visualize_network",
        default=False,
        action="store_true",
        help="TRT Networks will be exported to ONNX prior to Engine build for debugging. ",
    )
    parser.add_argument(
        "--dry_run",
        default=False,
        action="store_true",
        help="Run through the build process except the actual Engine build for debugging. ",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Specify whether offloading weights to CPU and streaming loading at runtime.",
    )
...
def build_model(
    ...
    **kwargs,
    ...
    if (
        not model_config.quant_mode.has_any_quant()
    ...
    assert (
        not build_config.plugin_config.streamingllm
        or architecture == "LlamaForCausalLM"
    ...
    if build_config.auto_parallel_config.enabled:
        ...
        assert (
            model_config.mapping.pp_size == 1 and model_config.mapping.tp_size == 1
...
def build_and_save(
    ...
    **kwargs,
    ...
    engine = build_model(
        build_config, rank, ckpt_dir, model_config, model_cls=model_cls, **kwargs
    )
    ...
    **kwargs,
    ...
    if workers == 1:
        ...
        with ProcessPoolExecutor(
            mp_context=get_context("spawn"), max_workers=workers
        ) as p:
            futures = [
                p.submit(
                    build_and_save,
            ...
            assert (
                len(exceptions) == 0
            ), "Engine building failed, please check error log."
...
def main():
    ...
    source = (
        args.checkpoint_dir if args.checkpoint_dir is not None else args.model_config
    )
    parallel_build(
        source,
        build_config,
